...
okt = Okt()
ko_model = gensim.models.Word2Vec.load('./data/ko/ko.bin').wv
...
emb_file = os.path.join(
    DATASET_DIR, 'embedded_features_word2vec_inference.csv')
if not os.path.exists(emb_file):
    # # embedding & make file
    ff = open(os.path.join(
        DATASET_DIR, 'embedded_features_word2vec_inference.csv'), 'a', newline='')
    writer_ff = csv.writer(ff)

    sent_max_len = 50
    for ix in range(len(essays)):
        for jx in range(len(essays[ix])):
            sent_embedded = []
            for w in essays[ix][jx]:
                try:
                    sent_embedded.append(ko_model[w])
                except KeyError:
                    zlist = [0 for i in range(200)]
                    sent_embedded.append(zlist)
            avg_sent_embedded = []
...
embedded_essay_raw = pd.read_csv(os.path.join(
    DATASET_DIR, 'embedded_features_word2vec_inference.csv'), encoding='cp949')
embedded_essay = []
...
result_file = os.path.join(SAVE_DIR, 'lstm_result.csv')
