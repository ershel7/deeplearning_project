...
embedded_essay_raw = pd.read_csv(os.path.join(
    DATASET_DIR, 'embedded_features_word2vec_holistic.csv'), encoding='cp949')
embedded_essay = []
...
sentence_model.fit(train_gen, steps_per_epoch=train_steps, validation_steps=valid_steps,
                   epochs=50, callbacks=[training_callback, early_stopping, model_checkpoint])
# 최종 모델 저장
sentence_model.save('final_model.h5')
